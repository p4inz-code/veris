/**
 * Script to repair package.json clean scripts and normalize metadata.
 * Handles the nested quote escaping issue in the "clean" scripts.
 */
...
for (const pkg of packages) {
  ...
  // Fix the clean script: replace broken nested quoting with single-quote version
  // Pattern: "node -e "fs.rmSync(\"dist\",{recursive:true,force:true})""
  // Fixed:  "node -e \"fs.rmSync('dist',{recursive:true,force:true})\""
  raw = raw.replace(
    /"clean":\s*"node -e "[^"]*fs\.rmSync\([^)]*\)[^"]*""/g,
